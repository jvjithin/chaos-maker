...
import { formatStepTitle, shouldEmitStep } from '../src/format-event';
...
describe('shouldEmitStep', () => {
  ...
  it('skips unapplied events in non-verbose mode', () => {
    const e = mkEvent({ applied: false });
    expect(shouldEmitStep(e, false)).toBe(false);
  });

  it('emits unapplied events in verbose mode', () => {
    const e = mkEvent({ applied: false });
    expect(shouldEmitStep(e, true)).toBe(true);
  });
