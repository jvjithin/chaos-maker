...
`graphqlOperation` is an additional matcher. It matches an `operationName` field, a named `query` / `mutation` / `subscription`, or a persisted-query GET parameter. Anonymous operations do not match a non-null matcher. Multipart and streaming bodies emit a diagnostic event with `reason: 'graphql-body-unparseable'`.
...
Use `applied` to distinguish "matched and rolled false" from "chaos actually changed browser behavior". SSE events include `detail.eventType`; GraphQL-aware network events include `detail.operationName` when one was identified.
