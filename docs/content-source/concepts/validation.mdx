...
Pass `onDeprecation: (issue) => …` to receive a `ValidationIssue` whenever a deprecated field is set. The registry is empty for v0.5.0 - the rails exist so the first deprecation in v0.5.x is a one-line addition.
