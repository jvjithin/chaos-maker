...
`ruleId` is a **positional** identifier derived from `<ruleType>#<index>` within the current config. Reordering rules between runs changes the IDs - that's acceptable for in-test diagnostics. Persistent IDs across config evolution are out of scope for v0.6.0.
